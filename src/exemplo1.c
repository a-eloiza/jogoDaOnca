#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "tabuleiro/tabuleiro.h"
#include "jogo/jogo.h"

#define MAXSTR 512
#define MAXINT 16

int main(int argc, char **argv) {
    char buf[MAXSTR];
    char tabuleiro[MAXSTR]; 
    char lado_meu;
    char lado_adv;
    char tipo_mov_adv;
    int num_mov_adv;
    int mov_adv_l[MAXINT];
    int mov_adv_c[MAXINT];
    int i;

    // conecta com o controlador do campo
    tabuleiro_conecta(argc, argv);

    // recebe o campo inicial e o movimento do adversario
    tabuleiro_recebe(buf);

    // separa os elementos do string recebido
    sscanf(strtok(buf, " \n"), "%c", &lado_meu);
    sscanf(strtok(NULL, " \n"), "%c", &lado_adv);
    sscanf(strtok(NULL, " \n"), "%c", &tipo_mov_adv);
    if(tipo_mov_adv == 'm') {
        num_mov_adv = 1;
        for(i = 0; i <= num_mov_adv; i++) {
            sscanf(strtok(NULL, " \n"), "%d", &(mov_adv_l[i]));
            sscanf(strtok(NULL, " \n"), "%d", &(mov_adv_c[i]));
        }
    } else if(tipo_mov_adv == 's') {
        sscanf(strtok(NULL, " \n"), "%d", &num_mov_adv);
        for(i = 0; i <= num_mov_adv; i++) {
            sscanf(strtok(NULL, " \n"), "%d", &(mov_adv_l[i]));
            sscanf(strtok(NULL, " \n"), "%d", &(mov_adv_c[i]));
        }
    }
    strncpy(tabuleiro, strtok(NULL, "."), MAXSTR-1);

    // mostra o que recebeu
    printf("%c\n", lado_meu);
    printf("%c %c", lado_adv, tipo_mov_adv);
    if(tipo_mov_adv != 'n') {
        if(tipo_mov_adv == 's') 
            printf(" %d", num_mov_adv);
        for(i = 0; i <= num_mov_adv; i++) {
            printf(" %d", mov_adv_l[i]);
            printf(" %d", mov_adv_c[i]);
        }
    }
    printf("\n");
    printf("%s", tabuleiro);
  
    jogada_t jogadas[50];
    int total_jogadas = 0;

    total_jogadas = gerar_movimentos(tabuleiro, lado_meu, jogadas);

    printf("\nForam encontradas %d jogadas possíveis para o lado '%c':\n", total_jogadas, lado_meu);
    for (i = 0; i < total_jogadas; i++) {
        char saida[MAXSTR];
        formatar_jogada(saida, lado_meu, jogadas[i]);
        printf("%s", saida);
    }
    
    // prepara um string com o movimento
    if(lado_meu == 'o')
        sprintf(buf, "o m 3 3 4 3\n");
    else
        sprintf(buf, "c m 3 5 4 4\n");

    // envia o movimento para o controlador do campo
    tabuleiro_envia(buf);  
}
